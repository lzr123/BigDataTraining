...
def main(argv):
    ...
    for video_type_code in range(10, 15):
        ...
        json_data = {'type_code': video_type_code, 'count': hist[0].tolist(), 'bucket': hist[1].tolist()}
        ...
        with open('./emotion_analyse.json', 'a') as res:
            json.dump(json_data, res, ensure_ascii=True)
            res.write('\n')
